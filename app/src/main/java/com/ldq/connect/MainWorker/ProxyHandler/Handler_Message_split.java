...
public class Handler_Message_split {
    ...
    public static void ShowSet(Context context) {
        try {
            ...
            ed.setId(2333);
            ed.setHint("支持变量{CHAR}");
            ed.setGravity(Gravity.NO_GRAVITY);
            ed.setSingleLine(false);
            ed.setText(MConfig.Get_String("Plugin", "Message_split", "Rule"));
            ...
            new AlertDialog.Builder(context, 3)
                    ...
                    .setNegativeButton("保存", (dialog, which) -> {
                        ...
                        MConfig.Put_List("Plugin", "Message_split", "Open", NewList);
                        MConfig.Put_String("Plugin", "Message_split", "Rule", ed.getText().toString());
                    }).show();
        } catch (Exception e) {
    ...
    public static void Callback(Object Message) {
        ...
        if (TroopConfig.size() > 0) {
            if (Message.getClass().getName().contains("MessageForText")
                    || Message.getClass().getName().contains("MessageForLongTextMsg")
                ...
                try {
                    ...
                    if ((Type == 1 && TroopConfig.contains(FriendUin)) || ((Type != 1 && Type != 10014) && TroopConfig.contains("-1"))) {
                        ...
                        String Rule = MConfig.Get_String("Plugin", "Message_split", "Rule");

                        Log.i("WHY", "Rule" + Rule);

                        String result = "";
                        boolean contains = Rule.contains("{CHAR}");
                        for (char c : msg.toCharArray()) {
                            if (!contains) {
                                result += Rule + c;
                            } else {
                                result += Rule.replace("{CHAR}", c + "");
                            }
                        }
                        msg = result;
