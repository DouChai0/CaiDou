	...
	implements Runnable, ConsoleInterface, Serializable
{
        ...
        while(!eof)
        {
            ...
            try
            {
				...
				*/
                ...
                final EvalError evalError = new EvalError("源文件: " + sourceFileInfo + " internal Error: " + e.getMessage(), node, callstack);
				...
				if ( e.getNode()==null )
					e.setNode( node );
				e.reThrow("正在加载的文件: "+sourceFileInfo);
            } catch ( EvalError e) {
                if ( DEBUG)
                    e.printStackTrace();
				// failsafe, set the Line as the origin of the error.
                ...
                final EvalError evalError = new EvalError("正在加载的文件: " + sourceFileInfo + " 未知错误: " + e.getMessage(), node, callstack);
                evalError.initCause(e);
                throw evalError;
            } catch(TokenMgrError e) {
                final EvalError evalError = new EvalError("正在加载的文件: " + sourceFileInfo + " 节点解析错误: " + e.getMessage(), node, callstack);
                evalError.initCause(e);
                throw evalError;
            } finally {
	...
	*/
	...
	{
		try {
			return (String)eval("getBshPrompt()","Memory");
		} catch ( Exception e ) {
			return "bsh % ";
		}
	}
