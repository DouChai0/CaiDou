		...
		throws UtilEvalError
	{
		...
		*/
		...
		if ( evalBaseObject instanceof Primitive)
			throw new UtilEvalError("不能将基本数据类型作为对象使用. "+
			"解析代码时: "+value);
		...
		*/
		...
		throw new UtilEvalError(
			"无法访问成员: " + field + ", 位于对象: " + evalBaseObject);
	}
	...
	*/
		...
		throws UtilEvalError
	{
		...
		if ( evalName == null && obj instanceof ClassIdentifier)
			throw new UtilEvalError("无法访问类实例: " + value );
        ...
        throws EvalError/*, ReflectError, InvocationTargetException*/
    {
		...
		if ( commandObject instanceof Class )
			try {
				...
				throw e.toEvalError("调用指令错误: ",
				callerInfo, callstack );
			}

		throw new InterpreterError("无效指令类型");
    }
